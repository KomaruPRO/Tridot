    ...
    public static void addEffectsTooltip(ImmutableList<MobEffectInstance> effects, List<Component> pTooltips, float pDurationFactor, float chance) {
        if (!effects.isEmpty()) {
            if (chance > 0 && chance < 1) {
                pTooltips.add(Component.translatable("tooltip.tridot.applies_with_chance", String.format("%.1f%%", chance * 100)).withStyle(ChatFormatting.GRAY));
            } else {
                pTooltips.add(Component.translatable("tooltip.tridot.applies").withStyle(ChatFormatting.GRAY));
            }
