...
public class BasicBossbar extends AbstractBossbar{
    ...
    public void render(LerpingBossEvent currentBossbar, BossEventProgress bossbarEvent, int offset, GuiGraphics pGuiGraphics, AbstractBossbar abstractBossbar, Minecraft mc){
        int screenWidth = pGuiGraphics.guiWidth();
        if(ClientConfig.BOSSBAR_TITLE.get()){
            bossbarEvent.setIncrement(32);
            renderBossbar(currentBossbar, offset, screenWidth, pGuiGraphics, abstractBossbar, mc);
            int nameX = screenWidth / 2 - mc.font.width(currentBossbar.getName()) / 2;
            int nameY = offset + 30;
            pGuiGraphics.drawString(mc.font, currentBossbar.getName(), nameX, nameY, 16777215);
        }else{
            bossbarEvent.setIncrement(26);
            renderBossbar(currentBossbar, offset, screenWidth, pGuiGraphics, abstractBossbar, mc);
        }
    }

    public void renderBossbar(LerpingBossEvent event, int offset, int screenWidth, GuiGraphics pGuiGraphics, AbstractBossbar abstractBossbar, Minecraft mc){
