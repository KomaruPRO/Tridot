...
public class MultiCreativeTabBuilder extends CreativeModeTab.Builder {
    public static ResourceLocation STANDARD_SUB_ARROWS = Tridot.ofTridot("textures/gui/subtab_arrows.png");
    ...
    public MultiCreativeTabBuilder setMainSubTab(SubCreativeTab subTab) {
        mainSubTab = subTab;
        return this;
    }

    public MultiCreativeTabBuilder withSubTabs(SubCreativeTab... subTab) {
        subCreativeTabs.addAll(Arrays.asList(subTab));
        return this;
    }
    ...
    public MultiCreativeTab build() {
        if ((this.type == CreativeModeTab.Type.HOTBAR || this.type == CreativeModeTab.Type.INVENTORY) && this.displayItemsGenerator != EMPTY_GENERATOR) {
            ...
            if(mainSubTab != null){
                creativeTab.mainSubtab = this.mainSubTab;
            } else {
                creativeTab.mainSubtab = this.subCreativeTabs.get(0);
            }

            for(SubCreativeTab tab : subCreativeTabs){
                creativeTab.addSubTab(tab);
            }
