package pro.komaru.tridot.oclient.graphics.render;
...
import pro.komaru.tridot.oclient.graphics.render.trail.TrailPoint;
import pro.komaru.tridot.oclient.graphics.render.trail.TrailRenderPoint;
import pro.komaru.tridot.outilities.Utils;
...
public class RenderBuilder{
    ...
    public int l1 = Utils.Render.FULL_BRIGHT;
    public int l2 = Utils.Render.FULL_BRIGHT;
    public int l3 = Utils.Render.FULL_BRIGHT;
    ...
    public RenderBuilder renderWavyQuad(PoseStack stack, float width, float height, float strength, float time){
        Vector3f[] positions = new Vector3f[]{new Vector3f(-1, 1, 0), new Vector3f(1, 1, 0), new Vector3f(1, -1, 0), new Vector3f(-1, -1, 0)};
        Utils.Render.applyWobble(positions, strength, time);
        return renderQuad(stack, positions, width, height);
    }
    ...
    public RenderBuilder renderTrail(Matrix4f pose, List<TrailPoint> trailSegments, Function<Float, Float> widthFunc, Consumer<Float> vfxOperator){
        ...
        for(int i = 1; i < count; i++){
            ...
            points[i] = new TrailRenderPoint(current, Utils.Render.perpendicularTrailPoints(previous, next, width));
        }
    ...
    public RenderBuilder renderDragon(PoseStack stack, double x, double y, double z, float radius, float partialTicks, float randomF){
        ...
        float rotation = (ClientTick.ticksInGame + partialTicks) / 200;
    ...
    public RenderBuilder renderSphere(PoseStack stack, float radius, int longs, int lats, float endU, boolean stretch){
        ...
        for(int i = 0; i < longs; ++i){
            for(int j = 0; j < lats; ++j){
                ...
                Vector3f p0 = Utils.Render.parametricSphere(u, v, radius);
                Vector3f p1 = Utils.Render.parametricSphere(u, vn, radius);
                Vector3f p2 = Utils.Render.parametricSphere(un, v, radius);
                Vector3f p3 = Utils.Render.parametricSphere(un, vn, radius);
