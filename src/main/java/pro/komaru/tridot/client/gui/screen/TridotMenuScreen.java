...
import pro.komaru.tridot.client.gui.components.TridotLogoRenderer;
import pro.komaru.tridot.core.config.ClientConfig;
import pro.komaru.tridot.utilities.client.Clr;

import java.util.*;

public class TridotMenuScreen extends Screen{
    public Screen lastScreen;
    public CubeMap CUBE_MAP = new CubeMap(new ResourceLocation(TridotLib.ID, "textures/gui/title/background/panorama"));
    ...
    public static List<TridotMod> mods = new ArrayList<>();
    public static List<TridotPanorama> panoramas = new ArrayList<>();
    ...
    public TridotMenuScreen(Screen lastScreen){
        super(Component.empty());
        this.lastScreen = lastScreen;
        this.logoRenderer = new TridotLogoRenderer(LOGO, false);
        if(lastScreen instanceof TitleScreen titleScreen){
            copyPanorama(titleScreen);
        }
        this.fadeInStart = Util.getMillis();

        mods = TridotModsHandler.getSortedMods();
        panoramas = TridotModsHandler.getSortedPanoramas();
    ...
    public void setLocalPanorama(TridotPanorama panorama){
    ...
    public static void drawPanoramaList(GuiGraphics gui, int x, int y, int mouseX, int mouseY, float partialTicks){
        ...
        for(int i = 0; i < 5; i++){
            ...
            TridotPanorama panorama = panoramas.get(index);
    ...
    public static List<Component> getDescription(TridotMod mod){
        ...
        if(mouseX >= x && mouseY >= y && mouseX <= x + 120 && mouseY < y + 100){
            for(int i = 0; i < 5; i++){
                ...
                if(mouseX >= x + 2 && mouseY >= y + 2 + (i * 20) && mouseX <= x + 102 && mouseY < y + 22 + (i * 20)){
                    ...
                    TridotModsHandler.setPanorama(panoramas.get(selectedPanorama));
