package github.iri.tridot.utilities.math;
...
public interface Interp extends Serializable {
    ...
    HashMap<Interp,String> invTable = Structs.revMap(table);

    float apply(float v);
    default float apply(float v, float min, float max) {
        return Mathf.lerp(min,max,apply(v));
    }
