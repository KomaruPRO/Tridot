...
public class Rand extends Random{
    ...
    private static final Rand[] instances = new Rand[32];
    public static Rand of(int instanceId) {
        if(instanceId < 0 || instanceId >= instances.length) throw new IllegalArgumentException("instanceId must be between 0 and "+(instances.length-1));
        Rand instance = instances[instanceId];
        if(instance == null){
            instance = new Rand();
